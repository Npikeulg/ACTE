...
def read_free_energies(volnum,num_unique,cell_data):
    ...
    if num_unique == 1:
        ...
        for file in files:
            ...
            latt_array[i][2] = float(l[6]) # c lattice
...
def find_CTE(volnum,num_unique,cell_data,latt_data):
    ...
    #polyorder  = 2   # must be less than the window
    tmin       = int(cell_data[23])
    tmax       = int(cell_data[24])
    tempsteps  = int(cell_data[25])
...
#            plt.close() 
        ...
        # take derivative using a gradient
        dera = np.gradient(ahat,tspacing)
        derc = np.gradient(chat,tspacing)
        
        #we will do it three times in a row...
        for i in range(10):
            dera = running_mean(dera,window)
...
def find_CP(volnum,num_unique,cell_data,lattder,bulkT,total_mass):
    ...
    if num_unique == 1:
        ...
        #integrate specific heat at constant pressure to get Enthalpy.
        enthalpy = it.cumtrapz(sheat[2][:],sheat[0][:],initial = 0)
        
        for i in range(sheat.shape[1]):
            sheat[3][i] = enthalpy[i]
...
def print_all(volnum,num_unique,cell_data,free_array,latt_data,lattder,bulkT,sheat):
    ...
    #print this data to a file
    f1= open('out.cv_cp_enthalpy','w')
    f1.write('# specific heat \n#temp \t cv \t cp\n' )
    for i in range(latt_data.shape[1]):
        f1.write('%s \t%s \t%s \t%s\n' %(sheat[0][i],sheat[1][i],sheat[2][i],sheat[3][i])) 
    f1.close()
    #generate gnuplot file 
    gen_GNUPLOT('out.cv_cp_enthalpy','specific_heat.gnuplot',3,'cv_cp')
...
def read_POSCAR():
    ...
    if diff_volumes == 6 and speccell == 0:
       for i in range(diff_volumes):
           ...
           avec = [np.float(avec[0])*an/a,np.float(avec[1])*an/a,np.float(avec[2])*an/a] 
           bvec = [np.float(bvec[0])*bn/b,np.float(bvec[1])*bn/b,np.float(bvec[2])*bn/b]
           cvec = [np.float(cvec[0])*cn/c,np.float(cvec[1])*cn/c,np.float(cvec[2])*cn/c] 
...
def determine_volumes():
    ...
    if ( np.abs(a-b)<=difftol and np.abs(b -c) <= difftol 
       ...
       speccell = 0
        ...
        and np.abs(float(bvec[0])) == np.abs(float(bvec[2])) 
        and np.abs(float(cvec[0])) == np.abs(float(cvec[1])) ):
       #a and b are the same, c is different, 36 volumes , ax, by, and cz are the only non-zero lattice parameters
       diff_volumes = 36
       speccell = 0
        ...
        and np.abs(float(bvec[0])) == np.abs(float(bvec[2])) 
        and np.abs(float(cvec[0])) == np.abs(float(cvec[1])) ):
       #a and c are the same, b is different, 36 volumes , ax, by, and cz are the only non-zero lattice parameters
       diff_volumes = 36
       speccell = 1
                    ...
                    if diff_volumes == 6:
                        ...
                        for i in range(diff_volumes):
                            ...
                            if os.path.isfile('lattice_'+str(i)+'/configs/outfile.free_energy'):
                                ...
                                if withDFTU0 == True:
                                    engtot,pressure = get_energy('lattice_'+str(i)+'/configs/outfile.U0')
                                else:
                        ...
                        for i in range(int(np.sqrt(diff_volumes))):
                            for j in range(int(np.sqrt(diff_volumes))):
                                ...
                                if os.path.isfile('lattice_'+str(cc)+'/configs/outfile.free_energy'):
                                    ...
                                    if withDFTU0 == True:
                                        engtot,pressure = get_energy('lattice_'+str(cc)+'/configs/outfile.U0')
                                    else:
